...
class TestUserProfileAPI:
    ...
    def test_get_user_profile_success(self, client, mock_auth_user):
        ...
        with patch("app.api.api_v1.endpoints.users.get_current_user") as mock_get_user:
            ...
            with patch(
                "app.services.auth_service.AuthService.get_user_by_auth0_id"
            ) as mock_get_by_auth0:
                mock_user = AsyncMock()
                mock_user.id = 1
    ...
    def test_update_user_profile_success(self, client):
        """测试成功更新用户档案"""
        update_data = {"username": "newusername", "timezone": "Asia/Shanghai"}
        ...
        with patch("app.api.api_v1.endpoints.users.get_current_user") as mock_get_user:
            ...
            with patch(
                "app.services.user_service.UserService.get_user_by_auth0_id"
            ) as mock_get_by_auth0:
                mock_user = AsyncMock()
                mock_user.id = 1
                mock_get_by_auth0.return_value = mock_user
...
class TestCORSConfiguration:
    ...
    def test_cors_actual_request(self, client):
        ...
        with patch("app.api.api_v1.endpoints.users.get_current_user") as mock_get_user:
            ...
            response = client.get("/api/v1/user/", headers={"Origin": "http://localhost:3000"})
