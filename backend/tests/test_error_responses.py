...
def test_authentication_error_format():
    ...
    # 验证认证错误包含适当的错误信息
    has_auth_error = (
        ("detail" in data and "auth" in str(data["detail"]).lower()) or
        ("error" in data) or
        ("message" in data and "auth" in str(data["message"]).lower()) or
        ("success" in data and data["success"] is False)
    )
...
def test_validation_error_format():
    ...
    try:
        ...
        # 其他类型的验证错误也是可接受的
        assert "field" in str(e).lower() or "required" in str(e).lower() or "missing" in str(e).lower()
