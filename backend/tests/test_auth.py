...
class TestJWTBearer:
    ...
    def mock_token(self, valid_token_payload):
        ...
        return jwt.encode(
            valid_token_payload,
            "test-secret",  # 在实际测试中会被RSA密钥替换
            algorithm="HS256"  # 简化测试
        )
...
class TestAuthService:
    ...
    async def test_get_or_create_user_existing(self, auth_service, user_info):
        ...
        auth_service.user_service.get_user_by_auth0_id = AsyncMock(
        ...
        auth_service.user_service.get_user_by_auth0_id.assert_called_once_with(
            "auth0|123456789"
        )
    ...
    async def test_get_or_create_user_new(self, auth_service, user_info):
        ...
        auth_service.user_service.create_user = AsyncMock(
            return_value=mock_user
        )
