...
class TestGitHubOAuthLogin:
    ...
    def test_github_oauth_callback_success(self, client, mock_db, mock_github_user_info, mock_user):
        ...
        with patch.object(GitHubOAuthService, 'exchange_code_for_token') as mock_exchange, \
             patch.object(GitHubOAuthService, 'get_user_info') as mock_get_user, \
             patch('app.services.user_service.UserService') as mock_user_service_class, \
             patch.object(JWTService, 'create_access_token') as mock_create_token:
            ...
            mock_user_service.create_or_update_github_user.assert_called_once_with(mock_github_user_info)
