...
class DailyRecordService:
    ...
    def get_by_user_and_date(db: Session, user_id: int, record_date: date) -> Optional[UserDailyRecord]:
        """根据用户ID和日期获取记录"""
        return db.query(UserDailyRecord).filter(
    ...
    @staticmethod
    def _update_record(db: Session, record: UserDailyRecord, record_data: UserDailyRecordCreate) -> UserDailyRecord:
        """更新现有记录"""
        # 更新主记录字段
        ...
        db.query(WorkoutSession).filter(
            WorkoutSession.user_daily_record_id == record.id
        ).delete()
    ...
    def add_workout_session(db: Session, record_id: int, workout_data: WorkoutSessionCreate) -> WorkoutSession:
        """为记录添加运动会话"""
        workout = WorkoutSession(
            user_daily_record_id=record_id,
            **workout_data.model_dump()
        )
        db.add(workout)
        db.commit()
    ...
    def get_recent_trends(db: Session, user_id: int, days: int = 7) -> dict:
        ...
        records = DailyRecordService.get_user_records(
        ...
        # 计算各维度平均值
        numeric_fields = [
            'sleep_quality', 'wake_clarity', 'calories', 'protein', 'fat', 'carbohydrates',
            'total_workout_duration', 'daily_steps', 'overall_mood', 'stress_level',
            'anxiety_level', 'energy_level', 'deep_work_hours', 'active_breaks',
            'focus_quality', 'task_completion', 'work_satisfaction',
