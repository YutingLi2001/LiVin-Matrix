...
def upgrade() -> None:
    ...
    # 创建每日记录表
    op.create_table('user_daily_records',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('created_at', sa.DateTime(), nullable=False),
        sa.Column('updated_at', sa.DateTime(), nullable=False),
        ...
        sa.Column('daily_steps', sa.Numeric(precision=4, scale=1), nullable=True, comment='每日步数（千步）'),
        # 情绪维度
        ...
        # 工作效率维度
        sa.Column('deep_work_hours', sa.Numeric(precision=3, scale=1), nullable=True, comment='深度工作时长 0-24'),
        ...
        sa.Column('work_environment', sa.String(length=20), nullable=True, comment='工作环境: home/office/cafe/mixed'),
        # 社交维度
        ...
        sa.Column('interpersonal_satisfaction', sa.Integer(), nullable=True, comment='人际关系满意度 1-10'),
        ...
        sa.CheckConstraint('carbohydrates >= 0 AND carbohydrates <= 1000', name='carbohydrates_range'),
        sa.CheckConstraint('daily_steps >= 0 AND daily_steps <= 50.0', name='daily_steps_range'),
        sa.CheckConstraint('deep_work_hours >= 0 AND deep_work_hours <= 24.0', name='deep_work_hours_range'),
        ...
        sa.CheckConstraint('focus_quality >= 1 AND focus_quality <= 10', name='focus_quality_range'),
        sa.CheckConstraint('initiated_social >= 0', name='initiated_social_positive'),
        sa.CheckConstraint('interpersonal_satisfaction >= 1 AND interpersonal_satisfaction <= 10', name='interpersonal_satisfaction_range'),
        ...
        sa.CheckConstraint('sleep_quality >= 1 AND sleep_quality <= 10', name='sleep_quality_range'),
        sa.CheckConstraint('solitude_satisfaction >= 1 AND solitude_satisfaction <= 10', name='solitude_satisfaction_range'),
        sa.CheckConstraint('stress_level >= 1 AND stress_level <= 10', name='stress_range'),
        sa.CheckConstraint('task_completion >= 1 AND task_completion <= 10', name='task_completion_range'),
        ...
        sa.CheckConstraint("work_environment IN ('home', 'office', 'cafe', 'mixed')", name='work_environment_valid'),
        sa.CheckConstraint('work_satisfaction >= 1 AND work_satisfaction <= 10', name='work_satisfaction_range'),
    ...
    op.create_index(op.f('ix_user_daily_records_record_date'), 'user_daily_records', ['record_date'], unique=False)
    op.create_index(op.f('ix_user_daily_records_user_id'), 'user_daily_records', ['user_id'], unique=False)
    ...
    op.create_table('workout_sessions',
        ...
        sa.Column('workout_type', sa.String(length=20), nullable=False, comment='训练类型: strength/cardio'),
        sa.Column('cardio_type', sa.String(length=20), nullable=True, comment='有氧类型: running/cycling/swimming/hiit/machine/other'),
        ...
        sa.CheckConstraint("(workout_type = 'cardio' AND cardio_type IS NOT NULL) OR workout_type = 'strength'", name='cardio_type_required'),
        sa.CheckConstraint("cardio_type IS NULL OR cardio_type IN ('running', 'cycling', 'swimming', 'hiit', 'machine', 'other')", name='cardio_type_valid'),
        ...
        sa.ForeignKeyConstraint(['user_daily_record_id'], ['user_daily_records.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_workout_sessions_id'), 'workout_sessions', ['id'], unique=False)
    op.create_index(op.f('ix_workout_sessions_user_daily_record_id'), 'workout_sessions', ['user_daily_record_id'], unique=False)
